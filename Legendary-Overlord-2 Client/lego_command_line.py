...
class CommandLine:
    def __init__(self):
        self.ser = ''
        self.port_given = False

    def interpret_input(self, line_in):
        try:
            ...
            if cmd == 'help' or cmd == '?':
                ...
                if not self.port_given:
                    ...
                    if params[1] == 'set':
                        if params[2] == 'level':
                            ...
                            self.ser.send_data(lego_arrays.set_dimmer_levels(targetDimmers, levels))
                        elif params[2] == 'pin':
                            values = segment_string(params[3], ',', type='NUM')
                            self.ser.send_data(lego_arrays.set_dimmer_property(targetDimmers, 'pin', values))
                        elif params[2] == 'enabled':
                            values = segment_string(params[3], ',', type='BOOLN')
                            self.ser.send_data(lego_arrays.set_dimmer_property(targetDimmers, 'enabled', values))
                        elif params[2] == 'bipolar':
                            values = segment_string(params[3], ',', type='BOOLN')
                            self.ser.send_data(lego_arrays.set_dimmer_property(targetDimmers, 'bipolar', values))
                        elif params[2] == 'inverse':
                            values = segment_string(params[3], ',', type='BOOLN')
                            self.ser.send_data(lego_arrays.set_dimmer_property(targetDimmers, 'inverse', values))
                        elif params[2] == 'method':
                            values = segment_string(params[3], ',', type='NUM')
                            self.ser.send_data(lego_arrays.set_dimmer_property(targetDimmers, 'method', values))
                        elif params[2] == 'function':
                            values = segment_string(params[3], ',', type='NUM')
                            self.ser.send_data(lego_arrays.set_dimmer_functions(targetDimmers, values))
                        elif params[2] == 'data':
                            data = segment_string(params[3], ',', type='NUM')
                            self.ser.send_data(lego_arrays.set_dimmer_data(targetDimmers, data))
                        else:
                ...
                self.port_given = True
                self.ser = lego_serial.LegoSerial(params[0])
            elif cmd == 'connect':
                if len(params) > 0:
                    self.port_given = True
                    self.ser = lego_serial.LegoSerial(params[0])
                    self.ser.open_serial()
                elif not self.port_given:
